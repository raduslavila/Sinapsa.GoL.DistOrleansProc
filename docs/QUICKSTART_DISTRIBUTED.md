...
- **NodePort services are not reachable on `localhost`** in multi-node Docker Desktop or Kind without `extraPortMappings`. The script always starts `kubectl port-forward` to bridge this gap.
- **ClusterId must be lowercase** (`k8s-gol`) to satisfy RFC 1123 / Kubernetes CRD naming rules.
- **Image pull policy** is `IfNotPresent` — if a node already has the image hash, it won't re-pull. After a rebuild you must explicitly load the new image into each node.
- The Orleans Dashboard at `/dashboard` is served on the same port as the API (5050), not on a separate port.
